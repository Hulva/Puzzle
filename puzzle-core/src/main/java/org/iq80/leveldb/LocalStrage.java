...
public class LocalStrage{
	...
	static File getPath(String keyspaces){
		File file;
		try {
			file = new File("data/STORAGE/" + java.net.URLEncoder.encode(keyspaces,"utf-8"));
		} catch (UnsupportedEncodingException e) {
			file = new File("data/STORAGE/" + keyspaces);
		}
		file.mkdirs();
		return file;
	}
	...
	public static List<String> getAllKeyspacesName() {
		File file = new File("data/STORAGE");
		List<String> nameList = new ArrayList<String>();
		File[] fiels = file.listFiles((f)->{return !f.getName().equals("SYSTEM");});
		if (null != fiels) {
			Stream.of(fiels).forEach((f) -> {nameList.add(f.getName());});
		}
		return nameList;
	}
