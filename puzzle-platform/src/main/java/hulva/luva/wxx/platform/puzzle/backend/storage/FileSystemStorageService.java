...
public class FileSystemStorageService implements StorageService {
	...
	public FileSystemStorageService() {
		this.rootLocation = Paths.get(Constants.PLUGIN_JAR_PATH_PRFIX);
		this.init();
	}
	...
	public void store(MultipartFile file, String fileName) {
		...
		try {
			if (!filename.endsWith(".jar")) {
				throw new StorageInvalidFileException(filename);
			}
			if (file.isEmpty()) {
	...
	public void init() {
		try {
			...
			Files.createDirectories(Paths.get(Constants.PLUGIN_Script_PATH_PRFIX));
		} catch (IOException e) {
